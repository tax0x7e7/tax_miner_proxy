...
2. 重构了抽水逻辑，更加平滑，提供收益优先或平滑优先的选项，新增-enable_profit_mode，-is_experiment_devfee
3. 修复了使用轻松矿工时潜在的崩溃bug
4. 新增了预设矿池的一键启动批处理，SSL_E_E_ETH.bat等
```
...
使用Config的用户在原来的config.yaml文件最后加入下文，并且替换二进制主程序即可
...
使用命令行的用户，在原本命令结尾加上下文，并且替换二进制主程序即可

```
-enable_profit_mode -is_experiment_devfee
```
